...
const Studio = lazy(() => import('../src/components/modals/CyberAuditStudioModal'));
function Harness() {
  const [open, setOpen] = useState(false);
  const [lastMissionId, setLastMissionId] = useState('');
  return (
    <div>
      <p data-testid="last-mission-id">{lastMissionId}</p>
      {open ? (
        <Suspense fallback={null}>
          <Studio onClose={() => setOpen(false)} onMissionUpdate={setLastMissionId} />
        </Suspense>
      ) : (
