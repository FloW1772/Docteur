...
try {
 ...
 for (const [width,height] of [[1920,1080],[1440,900],[1366,768],[1024,768],[768,1024],[390,844]]) {
  ...
  if(width>=768) {
   const shell=page.locator('.hud2-sidebar-shell');
   const toggle=page.getByRole('button',{name:'Réduire la navigation',exact:true});
   await toggle.focus(); await page.keyboard.press('Enter');
   check(await shell.getAttribute('data-sidebar-state')==='collapsed','collapsed state');
   check(await page.locator('.hud2-sidebar-content').isHidden(),'content hidden');
   check(await page.evaluate(()=>localStorage.getItem('docteur.sidebarCollapsed'))==='true','persist collapsed');
   const expand=page.getByRole('button',{name:'Déployer la navigation',exact:true});
   check(await expand.getAttribute('aria-expanded')==='false','announced collapsed');
   await expand.focus(); check(await page.getByRole('tooltip',{name:'Déployer la navigation'}).isVisible(),'focus tooltip');
   await page.keyboard.press('Escape'); check(await page.getByRole('tooltip',{name:'Déployer la navigation'}).isHidden(),'Escape tooltip');
   await page.mouse.move(600,350); await expand.hover();
   check(await page.getByRole('tooltip',{name:'Déployer la navigation'}).isVisible(),'hover tooltip');
   await page.reload({waitUntil:'domcontentloaded'}); await expand.waitFor();
   check(await shell.getAttribute('data-sidebar-state')==='collapsed','reload persistence');
   await page.locator('.hud2-sidebar-controls').getByRole('button',{name:'Rechercher',exact:true}).click();
   await page.getByRole('dialog',{name:'Search',exact:true}).waitFor(); checks++;
   await page.getByRole('button',{name:'Close',exact:true}).click();
   await expand.focus(); await page.keyboard.press('Enter');
   check(await shell.getAttribute('data-sidebar-state')==='expanded','expand keyboard');
   check(await page.evaluate(()=>localStorage.getItem('docteur.sidebarCollapsed'))==='false','persist expanded');
   await page.getByTestId('mode-toggle').click();
   check(await shell.getAttribute('data-sidebar-state')==='collapsed','Focus auto collapse');
   await expand.click(); check(await shell.getAttribute('data-sidebar-state')==='expanded','Focus manual expand');
   await page.keyboard.press('Escape'); check(await expand.evaluate(el=>el===document.activeElement),'Escape returns focus');
   await page.getByTestId('mode-toggle').click();
   check(await shell.getAttribute('data-sidebar-state')==='expanded','Dashboard restores preference');
  } else {
   check(await page.locator('.hud2-sidebar-controls').isHidden(),'no mobile rail');
   check(await page.locator('.hud2-sidebar-content').isVisible(),'mobile contents remain visible');
  }
 ...
 console.log(`V2.1 FINAL QA PASS ${checks}/${checks}`);
