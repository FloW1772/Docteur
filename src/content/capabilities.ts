...
export const HELP_DIRECTORY: HelpCategory[] = [
  ...
  },
  {
    ...
    items: [
      ...
      {
        name: 'MAÎTRE',
        description: 'Réponse aux incidents de sécurité locale — consulte les incidents/événements/preuves déjà collectés (par Observateur, Windows Defender, l\'Observateur d\'événements Windows) et propose des actions de remédiation précises, jamais exécutées automatiquement. Chaque action suit un flux obligatoire proposition → politique → approbation explicite → exécution : LEVEL 1 (collecte de preuves, analyse Defender ciblée) peut s\'exécuter directement après proposition ; LEVEL 2 (arrêt d\'un processus précis, blocage pare-feu d\'une IP précise, désactivation d\'une entrée de démarrage automatique) nécessite une approbation explicite ; LEVEL 3 (isolation réseau de l\'hôte, restauration réseau) nécessite en plus une confirmation renforcée distincte — jamais déduite de l\'ouverture d\'une fenêtre, d\'un survol de bouton ou d\'une confirmation précédente. L\'isolation réseau bloque le trafic entrant/sortant hors boucle locale via des règles pare-feu MAÎTRE dédiées et réversibles (jamais un vrai air-gap, jamais de modification des adaptateurs réseau, DNS, routes ou proxy) ; elle préserve toujours l\'accès local à Docteur (127.0.0.1) et peut être restaurée à tout moment via une action dédiée. Aucune élévation de privilèges automatique : si une action nécessite les droits administrateur et qu\'ils sont absents, elle échoue proprement (« access_denied ») plutôt que de contourner la protection. Un analyste local optionnel (déterministe, ou via Ollama local si configuré) sépare toujours faits observés, hypothèses et inconnues — une hypothèse n\'est jamais présentée comme une preuve, et ne peut jamais modifier la sévérité ou déclencher une action. Limites connues : Windows uniquement ; la mise en quarantaine via Defender n\'est pas prise en charge en V1 (ciblage impossible avec l\'API disponible) ; l\'analyse Defender rapide/complète n\'est pas prise en charge en V1 (durée non bornée) ; certaines actions nécessitent des droits administrateur ; l\'isolation réseau réelle n\'a pas été testée sur une machine physique en conditions réelles (seulement en environnement simulé/contrôlé) ; aucune remédiation automatique n\'existe — l\'approbation humaine reste requise pour toute action de niveau 2 ou 3.',
        feature: 'maitre', state: 'local',
        keywords: ['maitre', 'incident', 'remediation', 'isolation', 'pare-feu', 'firewall', 'defender', 'quarantaine', 'processus', 'persistance', 'approbation', 'reponse', 'securite'],
      },
    ],
  },
];
