...
import { DeviceFabricSettingsTab } from '../settings/DeviceFabricSettingsTab';
...
export type Tab = 'models' | 'stats' | 'privacy' | 'rassilon' | 'devices' | 'vocal' | 'inbox' | 'files' | 'audio' | 'external' | 'images' | 'memory' | 'connections' | 'video';
    ...
    <div className="modal-backdrop" onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-center gap-3 px-5 py-4" style={{ borderBottom: '1px solid rgba(61,255,170,0.1)' }}>
          ...
          <div className="flex-1">
            <h3 className="font-grotesk font-semibold text-base" style={{ color: '#f0eaff' }}>
              {tab === 'vocal' ? 'Paramètres — Voix & microphone' : tab === 'rassilon' ? 'RASSILON — Calcul local & LAN sécurisé' : tab === 'devices' ? 'DEVICES — Inventaire Device Fabric' : 'Paramètres — Router intelligent'}
            </h3>
            <p className="font-mono text-xs mt-0.5" style={{ color: '#7a6c9a' }}>
              {tab === 'vocal' ? 'Alt+M · Dictée, commandes et lecture' : tab === 'rassilon' ? 'Autorité locale · consentement visible · STOP prioritaire' : tab === 'devices' ? 'Liens explicites · lecture seule des agents · aucun routage' : 'Ctrl+, · Sélection automatique du LLM selon la complexité'}
            </p>
          </div>
        ...
        <div className="flex overflow-x-auto" style={{ borderBottom: '1px solid rgba(61,255,170,0.08)', padding: '0 20px' }}>
          {(['models', 'images', 'video', 'connections', 'stats', 'privacy', 'rassilon', 'devices', 'memory', 'vocal', 'inbox', 'files', 'audio', 'external'] as const).map(t => (
            <button
              key={t}
              type="button"
              onClick={() => setTab(t)}
            ...
            >
              {t === 'external' ? 'AGENTS EXTERNES' : t === 'models' ? 'MODÈLES' : t === 'stats' ? 'STATISTIQUES' : t === 'privacy' ? 'CONFIDENTIALITÉ' : t === 'rassilon' ? 'RASSILON' : t === 'devices' ? 'APPAREILS' : t === 'memory' ? 'MÉMOIRE' : t === 'vocal' ? 'VOIX & MICRO' : t === 'inbox' ? 'INBOX' : t === 'files' ? 'FICHIERS' : t === 'images' ? 'IMAGES' : t === 'connections' ? 'CONNEXIONS' : t === 'video' ? 'STUDIO VIDÉO' : 'AUDIO'}
            </button>
          ))}
        </div>
        ...
        <div style={{ maxHeight: 480, overflowY: 'auto' }}>
          ...
          {tab === 'devices' && <DeviceFabricSettingsTab />}
          {tab !== 'rassilon' && tab !== 'devices' && loading && (
            <div className="flex items-center justify-center py-12">
              <RefreshCw size={16} className="animate-spin" style={{ color: '#3d3060' }} />
            </div>
          )}

          {tab !== 'rassilon' && tab !== 'devices' && !loading && error && (
